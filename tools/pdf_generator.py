...
def _build_styles():
    ...
    styles = {
        ...
        "score_big": ParagraphStyle(
            ...
            alignment=TA_CENTER, leading=38, spaceAfter=16,
        ),
    ...
    def cell(score, label):
        ...
        return [
            ...
            Paragraph(f"Grade {grade}", ParagraphStyle(
                "g", fontSize=12, textColor=grade_color, fontName="Helvetica-Bold",
...
def _materiality_table(results: list, styles: dict) -> Table:
    ...
    t = Table(rows, colWidths=[4.5*cm, 1.5*cm, 2.5*cm, 8.5*cm])
    t.setStyle(TableStyle([
...
def _scientific_table(data_points: list, styles: dict) -> Table:
    ...
    t = Table(rows, colWidths=[3.5*cm, 3.0*cm, 4.0*cm, 4.5*cm, 2.0*cm])
    t.setStyle(TableStyle([
        ("BACKGROUND", (0,0), (-1,0), DARK_BG),
        ("TEXTCOLOR", (0,0), (-1,0), WHITE),
        ("FONTNAME", (0,0), (-1,0), "Helvetica-Bold"),
...
def generate_pdf_report(scorecard: "IntegrityScorecard") -> str:
    ...
    story.append(Paragraph(
        "ESG Integrity Report | Generated by Jakub Kopczyński ESG Analysis Tool",
        styles["header"]
    ))
